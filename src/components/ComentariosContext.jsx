...
const ComentarioProvider = ({ children }) => {
  ...
  const [imagen, setImagen] = useState('');
   ...
   const GuardarUsuario= (n,i)=> useEffect(
        setNombre(n),
        setImagen(i)
    )
  ...
  const CargarComentarios =async () => {
    try {
      const response = await axios.get('http://127.0.0.1:8080/comentarios');
  ...
  return (
    <ComentarioContext.Provider value={{ comentarios, CargarComentarios, nombre, GuardarUsuario, imagen}}>
      {children}
